/*Copyright 2024 tingqian */

#ifndef _LINUX_PERF_H
#define _LINUX_PERF_H
...
struct PerfEventJsonDumper {
    ...
    static std::string& getDumpFileName() {
        static std::string dump_file_name = "perf_dump.json";
    ...
    void finalize() {
        ...
        // start dump
        fw.open(PerfEventJsonDumper::getDumpFileName(), std::ios::out);
        fw << "{\n";
        fw << "\"schemaVersion\": 1,\n";
        fw << "\"traceEvents\": [\n";
        fw.flush();
        ...
        std::cout << " to " << PerfEventJsonDumper::getDumpFileName() << std::endl;
    }
                ...
                if (items.size() == 2) {
                    if (items[0] == "dump") {
                        ...
                        for (auto& cpu : cpus) {
                            ...
                            std::string& fileName = PerfEventJsonDumper::getDumpFileName();
                            fileName += cpu;
                        }
                    } else {
                    ...
                    if (items[0] == "switch-cpu") {
                        ...
                        switch_cpu = true;
                        auto tmp_cpu_mask = cpu_mask;
                        CPU_ZERO(&tmp_cpu_mask);
                        if (sched_getaffinity(getpid(), sizeof(cpu_set_t), &tmp_cpu_mask)) {
                    ...
                    perror(LINUX_PERF_"wrong configue");
                }
            }
    ...
    PerfEventCtxSwitch() {
        ...
        if (is_enabled) {
            ...
            printf(LINUX_PERF_"sizeof(cpu_set_t):%lu: _SC_NPROCESSORS_ONLN=%ld CPU_COUNT=%d\n", sizeof(cpu_set_t),
                   number_of_processors, CPU_COUNT(&mask));
            ...
            for (int cpu = 0; cpu < static_cast<int>(sizeof(cpu_set_t) * 8); cpu++) {
                ...
                auto* ctx_switch_pmeta = reinterpret_cast<perf_event_mmap_page*>(mmap(NULL, mmap_length, PROT_READ | PROT_WRITE,
                                         MAP_SHARED, ctx_switch_fd, 0));
            ...
            my_pid = getpid();
            auto& fileName = PerfEventJsonDumper::getDumpFileName();
            fileName = fileName + std::string("_pid")  + std::to_string(my_pid);
            my_tid = gettid();
        }
    }
...
struct PerfEventGroup : public IPerfEventDumper {
    ...
    PerfEventGroup(const std::vector<Config> type_configs, CallBackEventArgsSerializer fn = {}) : fn_evt_args_serializer(
            fn) {
        for (auto& tc : type_configs) {
            ...
            snprintf(events.back().format, FORMATSIZE, "%%%lulu, ", strlen(tc.name));
        }
        // env var defined raw events
        ...
        std::cout << "sched_getcpu:" << cpuId << std::endl;
        enable_dump_json = PerfRawConfig::get().dump_on_cpu(cpuId);
        serial = 0;
        if (enable_dump_json) {
            std::cout << "register on cpuId:" << cpuId << std::endl;
    ...
    RETRY:
        ...
        if (ev.fd < 0) {
            if (!pev_attr->exclude_kernel) {
                std::cout <<  LINUX_PERF_ << "perf_event_open(type=" << pev_attr->type << " ,config=" << pev_attr->config << ") with exclude_kernel=0 failed ";
                std::cout << "due to /proc/sys/kernel/perf_event_paranoid is 2),  set exclude_kernel=1 and retry..." << std::endl;
                pev_attr->exclude_kernel = 1;
                goto RETRY;
            } else {
                std::cout <<  LINUX_PERF_ << "perf_event_open(type=" << pev_attr->type << " ,config=" << pev_attr->config << ") failed " << std::endl;
                abort();
            }
        }
        ioctl(ev.fd, PERF_EVENT_IOC_ID, &ev.id);
        ev.pmeta = reinterpret_cast<perf_event_mmap_page*>(mmap(NULL, mmap_length, PROT_READ | PROT_WRITE, MAP_SHARED, ev.fd,
                   0));
    ...
    std::vector<uint64_t> rdpmc(FN fn, std::string name = {}, int64_t loop_cnt = 0,
                                std::function<void(uint64_t, uint64_t*, char*&)> addinfo = {}) {
        int cnt = events.size();
        ...
        if (!name.empty()) {
            ...
            char log_buff[LOGBUFFSIZE];
            char* log = log_buff;
            log += snprintf(log, LOGBUFFSIZE, "\e[33m");
            for (int i = 0; i < cnt; i++) {
                log += snprintf(log, LOGBUFFSIZE, events[i].format, pmc[i]);
            }
            auto duration_ns = tsc2nano(tsc1 - tsc0);
            log += snprintf(log, LOGBUFFSIZE, "\e[0m [%16s] %.3f us", name.c_str(), duration_ns / 1e3);
            if (hw_cpu_cycles_evid >= 0) {
                log += snprintf(log, LOGBUFFSIZE, " CPU:%.2f(GHz)", 1.0 * pmc[hw_cpu_cycles_evid] / duration_ns);
                if (hw_instructions_evid >= 0) {
                    log += snprintf(log, LOGBUFFSIZE, " CPI:%.2f", 1.0 * pmc[hw_cpu_cycles_evid] / pmc[hw_instructions_evid]);
                }
                if (loop_cnt > 0) {
                    // cycles per kernel (or per-iteration)
                    log += snprintf(log, LOGBUFFSIZE, " CPK:%.1fx%d", 1.0 * pmc[hw_cpu_cycles_evid] / loop_cnt, loop_cnt);
                }
            }
            if (addinfo) {
                addinfo(duration_ns, &pmc[0], log);
            }
            log += snprintf(log, LOGBUFFSIZE, "\n");
            std::cout << log_buff;
        }
        return pmc;
    }
    ...
    void read(bool verbose = false) {
        ...
        if (read_format & PERF_FORMAT_TOTAL_TIME_ENABLED) {
            time_enabled = *readv++;
            if (verbose) {
                printf("time_enabled:\t%lu\n", time_enabled);
            }
        }
        if (read_format & PERF_FORMAT_TOTAL_TIME_RUNNING) {
            time_running = *readv++;
            if (verbose) {
                printf("time_running:\t%lu\n", time_running);
            }
        }
