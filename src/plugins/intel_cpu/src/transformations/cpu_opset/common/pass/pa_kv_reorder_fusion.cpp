...
std::shared_ptr<ov::op::v0::Parameter> get_parameter_by_name(const std::shared_ptr<ov::Model>& model,
...
bool PaKVReorderFusion::run_on_model(const std::shared_ptr<ov::Model>& model) {
    const auto model_name = model->get_friendly_name();
    const auto& rt_info = model->get_rt_info();
    auto kv_update_marker = rt_info.find("auxiliary_kv_update_model");
    const bool has_marker = kv_update_marker != rt_info.end() && kv_update_marker->second.as<bool>();
    if (!has_marker && model_name != "kv_cache_reorder_model") {
        return false;
    }

    auto block_indices_begins = get_parameter_by_name(model, "block_indices_begins");
    auto block_update_indices_begins = get_parameter_by_name(model, "block_update_indices_begins");
    ...
    for (const auto& [index, key_path] : key_paths) {
        ...
        auto pa_kv_reorder = std::make_shared<ov::intel_cpu::PaKVReorderNode>(key_path.cache->output(0),
