...
#    endif
    ...
    class CustomThreadLocal : public ov::threading::ThreadLocal<std::shared_ptr<Stream>> {
    public:
        ...
        std::shared_ptr<Stream> local() {
            ...
            // fix the memory leak issue that cpuStreamer is already released, but still exists thread id in t_stream_count_map
