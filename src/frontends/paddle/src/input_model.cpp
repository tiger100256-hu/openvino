...
class InputModel::JsonInputModelImpl : InputModel::InputModelImpl {
public:
    template <typename T>
...
std::vector<std::shared_ptr<OpPlace>> InputModel::JsonInputModelImpl::get_op_places(const int32_t blck_idx) const {
    if (m_graph_changed) {
        return determine_cut_nodes();
    }
    if (static_cast<size_t>(blck_idx) < m_op_places.size())
...
std::vector<std::shared_ptr<OpPlace>> InputModel::JsonInputModelImpl::determine_cut_nodes() const {
    std::queue<OpPlace*> q;
    std::unordered_set<OpPlace*> visited;
    std::vector<std::shared_ptr<OpPlace>> new_op_places;
    new_op_places.reserve(m_op_places[0].size());
...
// load_consts with folder is compatible with old PaddlePaddle API.
template <typename T>
void InputModel::JsonInputModelImpl::load_consts(const std::basic_string<T>& folder_with_weights) {
    for (const auto& item : m_var_places) {
        const auto& var_desc = item.second->get_desc();
        const auto& name = item.first;
        if (ov::util::ends_with(name, std::string{"feed"}) || ov::util::ends_with(name, std::string{"fetch"}))
...
// load_consts with stream is compatible with new PaddlePaddle API.
void InputModel::JsonInputModelImpl::load_consts(std::istream* weight_stream) {
    for (const auto& item : m_var_places) {
        const auto& var_desc = item.second->get_desc();
        const auto& name = item.first;
        if (ov::util::ends_with(name, std::string{"feed"}) || ov::util::ends_with(name, std::string{"fetch"}))
...
class InputModel::JsonProgramDesc {
public:
    void ParseFromIstream (std::ifstream& pb_stream) {
        nlohmann::json data;
        data.parse(pb_stream);
        auto& base_code = data.at("base_code");
        m_graph.magic = base_code.at("image").template get<std::string>();
...
std::vector<Place::Ptr> InputModel::JsonInputModelImpl::get_inputs() const {
    return m_inputs;
}

std::vector<Place::Ptr> InputModel::JsonInputModelImpl::get_outputs() const {
    return m_outputs;
}

Place::Ptr InputModel::JsonInputModelImpl::get_place_by_tensor_name(const std::string& tensorName) const {
    if (m_var_places.count(tensorName))
        return m_var_places.at(tensorName);
    return nullptr;
}
...
void InputModel::JsonInputModelImpl::override_all_inputs(const std::vector<Place::Ptr>& inputs) {
    m_graph_changed = true;
    m_inputs.clear();
    for (const auto& inp : inputs) {
        m_inputs.push_back(castToTensorPlace(inp));
...
void InputModel::JsonInputModelImpl::override_all_outputs(const std::vector<Place::Ptr>& outputs) {
    m_graph_changed = true;
    m_outputs.clear();
    for (const auto& outp : outputs) {
        m_outputs.push_back(castToTensorPlace(outp));
...
void InputModel::JsonInputModelImpl::extract_subgraph(const std::vector<Place::Ptr>& inputs,
                                                  const std::vector<Place::Ptr>& outputs) {
    m_graph_changed = true;
    override_all_inputs(inputs);
    override_all_outputs(outputs);
...
void InputModel::JsonInputModelImpl::set_default_shape(Place::Ptr place, const ov::Shape& shape) {
    FRONT_END_NOT_IMPLEMENTED("set_default_shape");
}

void InputModel::JsonInputModelImpl::set_partial_shape(Place::Ptr place, const ov::PartialShape& p_shape) {
    castToTensorPlace(place)->set_partial_shape(p_shape);
}

ov::PartialShape InputModel::JsonInputModelImpl::get_partial_shape(Place::Ptr place) const {
    return castToTensorPlace(place)->get_partial_shape();
}

void InputModel::JsonInputModelImpl::set_element_type(Place::Ptr place, const ov::element::Type& type) {
    castToTensorPlace(place)->set_element_type(type);
}

ov::element::Type InputModel::JsonInputModelImpl::get_element_type(const Place::Ptr& place) const {
    return castToTensorPlace(place)->get_element_type();
}

void InputModel::JsonInputModelImpl::set_tensor_value(Place::Ptr place, const void* value) {
    m_graph_changed = true;
    auto tensor_place = castToTensorPlace(place);
    auto p_shape = tensor_place->get_partial_shape();
    auto type = tensor_place->get_element_type();
